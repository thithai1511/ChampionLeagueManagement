...
const TeamsManagement = () => {
  ...
  const getInviteTypeBadgeColor = (type) => {
    const colors = {
      ...
      'top8': 'bg-blue-100 text-blue-700 border-blue-200',
                        ...
                        <tr key={inv.invitationId} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2">
                              {inv.status === 'accepted' && (
                                <button
                                  onClick={() => handleUpdateInvitationStatus(inv.invitationId, 'qualified')}
                                  className="text-xs px-2 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors"
                                  title="Duyệt đủ điều kiện"
                                >
                                  Duyệt
                                </button>
                              )}
                              {(inv.status === 'pending' || inv.status === 'sent') && (
                                <>
                                  ...
                                  <button
                                    onClick={() => handleUpdateInvitationStatus(inv.invitationId, 'declined', 'Admin từ chối')}
                              ...
                              {(inv.status === 'declined' || inv.status === 'rejected' || inv.status === 'expired') && (
                                <span className="text-xs text-gray-400">Cần thay thế</span>
                              )}
                              {inv.status === 'qualified' && (
                                <span className="text-xs text-green-600 font-medium">✓ Đủ ĐK</span>
                              )}
                            </div>
                          </td>
                        </tr>
